...
.view-select-user .select2-selection,
.action-select-user .select2-selection {
	height: 38px;
...
.view-select-user .select2-selection__rendered,
.action-select-user .select2-selection__rendered {
	line-height: 38px !important;
}

.view-select-user .select2-selection__arrow,
.action-select-user .select2-selection__arrow {
...
<script>
	var _admin = (function(obj) {
		...
		onLoad : function() {
			...
			$(document).on("click",".view-role-save-btn", function(){
				var data = {
			...
			/*_admin.ajax("./getActionRole.ajax", null, function(data) {
				for (var i = 0; i < data.length; i++) {
					_admin.createUserRow(data[i]);
				}
			});*/
